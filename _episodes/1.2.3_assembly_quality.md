...
> need many resources. Assigning 2 CPUs and 1 GB of RAM for sbatch should be enough.
> The program should take just a few seconds to run.
> 
> ~~~
> # create a folder for the assessment within todays folder
> $ mkdir ./1.2_assembly/20_results_assessment_quast
>
...
> $ $quast -o ./1.2_assembly/20_results_assessment_quast/assembly ./1.2_assembly/10_results_assembly_flye/single_assemblies/assembly.fasta
> ~~~
> {: .language-bash}
>
...
>#SBATCH --output=./1.2_assembly/20_results_assessment_quast/assessment_quast.slurm.%j.out
>#SBATCH --error=./1.2_assembly/20_results_assessment_quast/assessment_quast.slurm.%j.err
>
># Set some variables for the quast script on draco and the files to be analysed
>quast='python3 /home/groups/VEO/tools/quast/v5.2.0/quast.py'
>assembly='./1.2_assembly/10_results_assembly_flye/single_assemblies/barcode62.fasta'
>outdir='./1.2_assembly/20_results_assessment_quast'
> ```
> {: .source}
{: .solution}
...
> to use (use the solution, if you're short on time). Assig 32 CPUs and 10 GB of RAM for the
> sbatch script. It should take less than a minute to run.
> 
> ~~~
> # create a folder for the assessment within todays folder
>$ mkdir ./1.2_assembly/30_results_alignment_minimap2
>
...
>#SBATCH --output=./1.2_assembly/30_results_alignment_minimap2/alignment_minimap2.slurm.%j.out
>#SBATCH --error=./1.2_assembly/30_results_alignment_minimap2/alignment_minimap2.slurm.%j.err
>
>
...
>
># assembly
>outdir='./1.2_assembly/30_results_alignment_minimap2'
>assembly='./1.2_assembly/10_results_assembly_flye/single_assemblies/barcode62.fasta'
>mkdir -p $outdir
>
>$minimap2 -x map-ont -a -t 30 $assembly $indir/barcode62_filtered_reads.fastq.gz | \
> $samtools view -u - | $samtools sort -o $outdir/barcode62.bam --write-index -
> $samtools stats $outdir/barcode62.bam > $outdir/barcode62_stats.txt
>
> ```
> {: .source}
{: .solution}
...
> You now have access to several metrics about your assembly.
> 
...
> 
> Can you summarize these results in a clear way and evaluate if the quality of the assembly is good or not? 
> 
>   1. How many contigs does your assembly have?
>   2. How many contigs does your assembly have above 10000 bp and above 50000 bp? There is a wide variation in genome length of phages, but many have around 40000 bp. Are most of your contigs likely complete genomes or not? Why do you think that? 
